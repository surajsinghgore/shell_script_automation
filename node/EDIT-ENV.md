...
Node.js has two kinds of variable, and they cost very different amounts to
change:

| | Where it lives | To apply a change | Takes |
|---|---|---|---|
| `environment variables` | compiled into the **browser bundle** at build time | full rebuild | 2–5 min |
...
**A `environment variables` changed:**
...
```
 ...
 6. warns about malformed lines and risky APP_ names
 7. asks to confirm
...
The one rule: **don't prefix a secret with `APP_`.** That prefix is what
tells Node.js to compile the value into the JavaScript everyone downloads. The
script warns if it sees a `APP_` name containing `SECRET`, `PASSWORD`,
`PRIVATE`, `_KEY` or `TOKEN`:

```
  ! A environment variables name looks secret — those are compiled into the browser
  ! bundle and readable by anyone. Drop the APP_ prefix to keep it server-side.
```
...
warns. For a Node.js app that's worse than for a static site: production and
