...
CONF="$SCRIPT_DIR/node-deploy.$ENV_NAME.conf"
ENVF="$SCRIPT_DIR/.env.$ENV_NAME"
APP_NAME="$(conf_value "$ENV_NAME" APP_NAME)"
APP_PORT="$(conf_value "$ENV_NAME" APP_PORT)"
...
cat <<EOF
  ...
  ${C_Y}On $SERVER_HOST${C_0}
    the pm2 process "$APP_NAME"            (frees port $APP_PORT)
...
if [[ "$DO_SERVER" =~ ^[Yy] ]]; then
  ...
  ssh -i "$PEM_PATH" -o StrictHostKeyChecking=accept-new -o ConnectTimeout=20 -o LogLevel=ERROR \
      ...
      "APP_NAME=$(printf '%q' "$APP_NAME") APP_DIR=$(printf '%q' "$APP_DIR") WEB_ROOT=$(printf '%q' "$WEB_ROOT") APP_PORT=$(printf '%q' "$APP_PORT") bash -s" <<'REMOTE'
set -uo pipefail
...
# Stop the Node process FIRST. Deleting its files out from under a running
# process leaves pm2 restart-looping a server.js that no longer exists.
...
if pm2 describe "$APP_NAME" >/dev/null 2>&1; then
  ...
  echo "no pm2 process named $APP_NAME"
fi
