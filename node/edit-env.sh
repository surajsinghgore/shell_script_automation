...
#  you to save and close it, shows what changed, then uploads it and restarts.
#
...
#
#  APPLYING CHANGES
#  A backend reads every variable at startup, so a pm2 restart applies them —
#  seconds, not minutes. There is no browser bundle to recompile.
#
#  Unlike the frontend toolchains, secrets ARE appropriate here: nothing in
#  this file reaches a browser.
# ==============================================================================
...
[[ -f "$DEPLOYER" ]] || die "node-deploy.sh not found next to this script."
...
[[ ${#ENVS[@]} -gt 0 ]] || die "No environments configured. Run: ./node-deploy.sh setup"

conf_value() { sed -n "s/^$2=//p" "$SCRIPT_DIR/node-deploy.$1.conf" 2>/dev/null | head -1 | tr -d "\"'"; }
...
if [[ ! -f "$ENV_PATH" ]]; then
  ...
  SEED=""
  for c in "$SCRIPT_DIR/.env" "$SCRIPT_DIR/.env.example"; do
    [[ -f "$c" ]] && { SEED="$c"; break; }
  done
  if [[ -n "$SEED" ]]; then
...
# reported below so you can see exactly what will take effect.
...
# A backend has no build-time/runtime split: nothing here is compiled into a
# browser bundle, so every variable is read from the environment when the
# process starts. A restart is always sufficient and takes seconds.
#
# The exception is a compile step that BAKES a value in — some TypeScript
# builds inline constants. If your BUILD_CMD does that, answer 'b'.
step "Applying to $ENV_NAME"
if [[ -n "$CHANGED_KEYS" ]]; then
  note "changed: $(printf '%s' "$CHANGED_KEYS" | tr '\n' ' ')"
fi
note "backend variables are read at startup — a restart applies them (seconds)"

read -r -p "  Restart $ENV_NAME now? (Y/n, or 'b' to rebuild and redeploy): " a
case "$a" in
  [Nn]*) warn "Saved locally but NOT applied. Run: ./edit-env.sh $ENV_NAME"; exit 0 ;;
  [Bb]*) "$DEPLOYER" env "$ENV_NAME"
         ok "$ENV_NAME rebuilt and redeployed with the new variables" ;;
  *)     ENV_MODE=restart "$DEPLOYER" env "$ENV_NAME"
         ok "$ENV_NAME restarted with the new environment" ;;
esac

# Anything here is server-side only, so real secrets are fine — but a leaked
# backend .env is a database, not a public URL. Worth the reminder.
if grep -qE '^[[:space:]]*[A-Za-z_]*(SECRET|PASSWORD|PRIVATE|_KEY|TOKEN|DATABASE_URL)' "$ENV_PATH"; then
  note "this file holds credentials — it is chmod 600 here and on the server, and gitignored"
fi
