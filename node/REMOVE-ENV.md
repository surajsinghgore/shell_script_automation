...
```
On your laptop
  node-deploy.<name>.conf                the environment's answers
...
Removing a Node.js environment frees noticeably more than a static one — the
...
| You want | Use |
|---|---|
| Undo a bad deploy | `./node-deploy.sh rollback <name>` — seconds, no rebuild |
| Change branch, domain or port | `./node-deploy.sh config <name>` |
