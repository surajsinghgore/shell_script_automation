...
#
#  Environments come from the react-develop.<name>.conf files that
#  ./react-develop.sh setup created — this script never invents its own.
# ==============================================================================
...
[[ -f "$DEPLOYER" ]] || die "react-develop.sh not found next to this script ($SCRIPT_DIR)."
...
[[ ${#ENVS[@]} -gt 0 ]] || die "No environments configured yet. Run: ./react-develop.sh setup"

conf_value() {  # conf_value <env> <KEY>
  sed -n "s/^$2=//p" "$SCRIPT_DIR/react-develop.$1.conf" 2>/dev/null \
    | head -1 | tr -d "\"'"
}
...
show_table() {
  ...
  for e in "${ENVS[@]}"; do
    printf '  %-4s %-24s %-12s %s\n' "$i)" "$e" \
      "$(conf_value "$e" GIT_BRANCH)" "$(conf_value "$e" DOMAIN)"
    i=$(( i + 1 ))
  done
  echo
}
...
for e in "${TARGETS[@]}"; do
  ...
  step "$e — branch $branch -> ${domain:-ip only}"
...
if [[ ${#DEPLOYED[@]} -eq 0 ]]; then
  ...
  note "Bad build live? ./react-develop.sh rollback <name>"
fi
