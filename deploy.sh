...
#     ./deploy.sh allow-ip  # point the SSH security group rule at your current IP
#     ./deploy.sh harden    # fail2ban, auto security updates, sshd lockdown, swap
#
...
SSL_EMAIL=""
SETUP_FIREWALL="y"
SG_ID=""             # sg-0abc123... (for ./deploy.sh allow-ip)
...
CONFIG_KEYS=(PEM_PATH SERVER_HOST SSH_USER NODE_MAJOR REPO_URL REPO_PRIVATE GIT_USER
             ...
             START_CMD ENV_FILE DOMAIN WWW_ALIAS SSL_EMAIL SETUP_FIREWALL SG_ID AWS_REGION)
...
BANNER
  ...
  [[ -n "$SG_ID" ]] && ask AWS_REGION "AWS region (e.g. ap-south-1)"
...
cat > "$APP_DIR/ecosystem.config.cjs" <<EOS
module.exports = {
  apps: [{
    name: "$APP_NAME",
...
set -euo pipefail
SERVER_NAMES="_"
ZONE="$(echo "$APP_NAME" | tr -c 'a-zA-Z0-9' '_' | cut -c1-24)"
if [ -n "$DOMAIN" ]; then
  SERVER_NAMES="$DOMAIN"
...
allow_ip() {
  ...
  [[ -n "$SG_ID" ]] || die "No SG_ID configured. Run ./deploy.sh config and enter your security group id."
