...
#
#  Environments come from the next-deploy.<name>.conf files that
#  ./next-deploy.sh setup created — this script never invents its own.
# ==============================================================================
...
[[ -f "$DEPLOYER" ]] || die "next-deploy.sh not found next to this script ($SCRIPT_DIR)."
...
[[ ${#ENVS[@]} -gt 0 ]] || die "No environments configured yet. Run: ./next-deploy.sh setup"

conf_value() {  # conf_value <env> <KEY>
  sed -n "s/^$2=//p" "$SCRIPT_DIR/next-deploy.$1.conf" 2>/dev/null \
    | head -1 | tr -d "\"'"
}
...
if [[ ${#DEPLOYED[@]} -eq 0 ]]; then
  ...
  note "Bad build live? ./next-deploy.sh rollback <name>"
fi
