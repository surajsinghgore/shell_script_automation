# Setup — from nothing to a live HTTPS Next.js site
...
| `output: "standalone"` | In `next.config.ts` — see below. |

```bash
cd /c/Users/suraj/Downloads/vps/next
```

### Your app needs standalone output
...
Without it there is no `.next/standalone/server.js` and the deploy stops with a
clear error rather than publishing something broken. Standalone keeps the
release small — a minimal `node_modules` instead of the full 400 MB tree.

### Budget the resources

A Next.js environment is a **running process**, not a folder of files:
...
```bash
./next-deploy.sh status        # once configured
# or straight from the server:
free -m ; df -h /
```

On a 1 GB / 8 GB box, one Next.js environment alongside an existing app is
...
`./next-deploy.sh ssl <env>` once it propagates.
...
```
.env.production    DATABASE_URL=...   NEXT_PUBLIC_SITE_URL=https://example.com
.env.develop       DATABASE_URL=...   NEXT_PUBLIC_SITE_URL=https://dev.example.com
```

Naming is `.env.<environment name>` — auto-detected, nothing to configure.

**Next.js splits variables in two**, and the difference matters:
...
don't carry the `NEXT_PUBLIC_` prefix. `edit-env.sh` warns if a `NEXT_PUBLIC_`
name looks secret.
...
```bash
./next-deploy.sh setup all        # every environment
./next-deploy.sh setup develop    # or one at a time
```
...
```
 ...
 6. Assemble the standalone release  ← server.js + .next/static + public
...
| `no response on :PORT` | `./next-deploy.sh logs <env>` — the app crashed on boot |
...
Full command list: `./next-deploy.sh help`
