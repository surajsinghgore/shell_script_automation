#!/usr/bin/env bash
# ==============================================================================
#  next-deploy.sh — one-command deploy for a server-rendered Next.js app.
#
#  Runs FROM YOUR LAPTOP. Talks to the server over SSH using your .pem key.
#  The build happens ON THE SERVER; pm2 runs the standalone server and nginx
#  reverse-proxies to it. Each deploy lands in its own release directory and a
#  symlink is flipped, so `rollback` is instant and needs no rebuild.
#
#  Usage:
#     ./next-deploy.sh setup     # first time: asks everything, installs stack
#     ./next-deploy.sh deploy    # every update: pull -> build -> publish -> reload
#     ./next-deploy.sh rollback  # back to the previous release (no rebuild)
#     ./next-deploy.sh releases  # list releases on the server
#     ./next-deploy.sh ssl       # issue/renew HTTPS cert for the domain
#     ./next-deploy.sh env       # upload .env, then rebuild or restart
#     ./next-deploy.sh logs      # tail live app logs (pm2)
#     ./next-deploy.sh status    # pm2 + nginx + release health
#     ./next-deploy.sh restart   # restart the Node process
#     ./next-deploy.sh nginx     # rewrite + reload the nginx server block
#     ./next-deploy.sh ssh       # drop into a shell on the server
#     ./next-deploy.sh config    # re-run the questions / edit answers
#     ./next-deploy.sh envs      # list configured environments
#     ./next-deploy.sh allow-ip  # point the SSH security group rule at your IP
#     ./next-deploy.sh harden    # fail2ban, auto updates, sshd lockdown, swap
#
...
#
#     ./next-deploy.sh setup            # asks how many, writes each config
#     ./next-deploy.sh setup all        # provision every environment
#     ./next-deploy.sh deploy develop   # act on one environment
#     ./next-deploy.sh deploy all       # act on every environment in turn
#
#  UNLIKE A REACT SPA: this app is a running Node process. Every environment
#  needs its OWN PORT, and server-side env vars take effect on restart without
#  a rebuild (only NEXT_PUBLIC_* are baked in at build time).
#
#  Answers are cached in ./next-deploy*.conf (chmod 600) — they hold your git
...
CONFIG_FILE="${CONFIG_FILE:-$SCRIPT_DIR/next-deploy.conf}"
ENV_NAME="${DEPLOY_ENV:-}"

list_envs() {
  ls -1 "$SCRIPT_DIR"/next-deploy.*.conf 2>/dev/null \
    | sed 's|.*/next-deploy\.||; s|\.conf$||' || true
}
...
resolve_config() {
  ...
  if [[ -n "$ENV_NAME" ]]; then
    CONFIG_FILE="$SCRIPT_DIR/next-deploy.$ENV_NAME.conf"
    if [[ ! -f "$CONFIG_FILE" && "${CMD:-}" != "setup" && "${CMD:-}" != "config" ]]; then
      die "No such environment '$ENV_NAME'. Configured: $(echo $envs). Run: ./next-deploy.sh setup"
    fi
    return 0
  fi
  if [[ -f "$SCRIPT_DIR/next-deploy.conf" && -z "$envs" ]]; then
    CONFIG_FILE="$SCRIPT_DIR/next-deploy.conf"; return 0
  fi
  local count; count="$(printf '%s\n' "$envs" | grep -c . || true)"
  if [[ "$count" == "1" ]]; then
    ENV_NAME="$envs"; CONFIG_FILE="$SCRIPT_DIR/next-deploy.$ENV_NAME.conf"; return 0
  fi
  if [[ "$count" -gt 1 ]]; then
    printf '\n  ! Several environments exist. Name one, or use "all":\n' >&2
    printf '      %s\n' $envs >&2
    printf '      e.g. ./next-deploy.sh %s %s\n\n' "${CMD:-deploy}" "$(printf '%s\n' "$envs" | head -1)" >&2
    exit 1
  fi
  CONFIG_FILE="$SCRIPT_DIR/next-deploy.conf"
}
...
INSTALL_CMD="npm ci || npm install"   # dev deps are build deps
...
CONFIG_KEYS=(PEM_PATH SERVER_HOST SSH_USER NODE_MAJOR REPO_URL REPO_PRIVATE GIT_USER
             ...
             BUILD_CMD ENV_FILE DOMAIN WWW_ALIAS SSL_EMAIL SETUP_FIREWALL SG_ID
...
save_config() {
  ...
  {
    echo "# next-deploy.sh config — contains a git token. NEVER commit this file."
...
# A Next.js app is a listening process, so the port genuinely has to be free.
port_taken_locally() {  # port_taken_locally <port> <this-env>
  local p="$1" me="$2" f e
  for f in "$SCRIPT_DIR"/next-deploy.*.conf; do
    [[ -f "$f" ]] || continue
    e="$(basename "$f" | sed 's|next-deploy\.||; s|\.conf$||')"
    [[ "$e" == "$me" ]] && continue
...
verify_repo_and_branch() {
  ...
  if [[ -z "$branches" ]]; then
    ...
    [[ "$mode" == "strict" ]] && die "Stopped before touching the server. Fix with: ./next-deploy.sh config"
    read -r -p "  Continue anyway? (y/N): " a
    [[ "$a" =~ ^[Yy] ]] || die "Stopped. Run ./next-deploy.sh config."
    return 0
  fi
  ...
  while ! printf '%s\n' "$branches" | grep -qxF -- "$GIT_BRANCH"; do
    ...
    [[ "$mode" == "strict" ]] && die "Nothing was deployed. Fix with: ./next-deploy.sh config"
...
# ------------------------------- ssh plumbing ---------------------------------
init_ssh() {
  [[ -n "$PEM_PATH" && -f "$PEM_PATH" ]] || die "PEM key not found: '$PEM_PATH'. Run: ./next-deploy.sh config"
...
remote() {
  ...
  for k in APP_NAME APP_DIR WEB_ROOT APP_PORT NODE_MAJOR REPO_URL GIT_BRANCH GIT_USER \
           GIT_TOKEN INSTALL_CMD BUILD_CMD DOMAIN WWW_ALIAS SSL_EMAIL SSH_USER \
...
wizard() {
  ...
  ┌──────────────────────────────────────────────┐
  │   Next.js (SSR)  ->  VPS   deployment setup  │
  └──────────────────────────────────────────────┘
BANNER
  ...
  [[ "$APP_NAME" == "nextapp" && -n "$guess" ]] && APP_NAME="$guess"
  ...
  step "4/7  Port"
  note "Next.js runs as a Node process — every environment needs its own port"
...
multi_env_wizard() {
  ...
  for (( i=1; i<=count; i++ )); do
    ...
    if (( i == 1 )); then
      CONFIG_FILE="$SCRIPT_DIR/next-deploy.$ENV_LABEL.conf"
      wizard
  ...
  Now provision them on the server:
     ...
     ${C_B}./next-deploy.sh setup all${C_0}          (or: setup <name>, one at a time)
...
# node_modules, but does NOT copy .next/static or public/ into it. Skip this and
# the site loads with no CSS, no JS chunks and no images.
[ -d "$APP_DIR/.next/standalone" ] || {
  echo "ERROR: .next/standalone missing."
  echo "       Add  output: 'standalone'  to next.config and redeploy."
  exit 1; }
...
mkdir -p "$NEW"
cp -a "$APP_DIR/.next/standalone/." "$NEW/"
mkdir -p "$NEW/.next"
cp -a "$APP_DIR/.next/static" "$NEW/.next/static"
[ -d "$APP_DIR/public" ] && cp -a "$APP_DIR/public" "$NEW/public"
[ -f "$NEW/server.js" ] || { echo "ERROR: no server.js in the release"; rm -rf "$NEW"; exit 1; }
...
server {
    ...
    # Content-hashed build assets: cache hard, and exempt from the rate limit —
    # one page view pulls dozens of them.
    ...
    # Next.js image optimiser output — cacheable, but not immutable.
...
setup_ssl() {
  ...
  [[ -n "$SSL_EMAIL" ]] || { warn "No email configured — skipping HTTPS. Run './next-deploy.sh config' then 'ssl'."; return 0; }
  step "Checking DNS for $DOMAIN"
  local resolved; resolved="$(resolve_ip "$DOMAIN")"
  if [[ -z "$resolved" ]]; then
    warn "$DOMAIN does not resolve yet. Point an A record to $SERVER_HOST, then: ./next-deploy.sh ssl"; return 0
...
[ -f "$WEB_ROOT/releases/$PREV/server.js" ] || { echo "ERROR: $PREV has no server.js"; exit 1; }
...
#   (default) full rebuild, because NEXT_PUBLIC_* are baked in at build time
upload_env() {
...
REMOTE
  ...
  grep -qE '^NEXT_PUBLIC_' "$src" 2>/dev/null && \
    warn "NEXT_PUBLIC_* values are compiled into the browser bundle and readable by anyone. Never put real secrets there."
...
REMOTE
      ...
      warn "NEXT_PUBLIC_* changes are NOT live — those need a rebuild: ./next-deploy.sh deploy ${ENV_LABEL:-}"
      ;;
    *)
      note "rebuilding — NEXT_PUBLIC_* values are baked into the bundle at build time"
      deploy_app ;;
  esac
}
...
allow_ip() {
  ...
  [[ -n "$SG_ID" ]] || die "No SG_ID configured. Run ./next-deploy.sh config."
...
summary() {
  ...
  Bad build went live?                 ${C_B}./next-deploy.sh rollback$suffix${C_0}
...
cmd_envs() {
  local envs; envs="$(list_envs)"
  if [[ -z "$envs" ]]; then
    [[ -f "$SCRIPT_DIR/next-deploy.conf" ]] && { echo "  single environment (next-deploy.conf)"; return 0; }
    warn "No environments configured yet. Run: ./next-deploy.sh setup"; return 0
  fi
  ...
  while read -r e; do
    [[ -z "$e" ]] && continue
    f="$SCRIPT_DIR/next-deploy.$e.conf"
...
cmd_setup() {
  ...
  Note: a Next.js environment is a running Node process — budget roughly
  250 MB of RAM and 600 MB of disk for each one.
...
if [[ "$ENV_NAME" == "all" ]]; then
  all_envs="$(list_envs)"
  [[ -n "$all_envs" ]] || die "No environments configured. Run: ./next-deploy.sh setup"
  while read -r e; do
    [[ -z "$e" ]] && continue
    printf '\n%s########  %s : %s  ########%s\n' "$C_B" "$CMD" "$e" "$C_0"
...
case "$CMD" in
  ...
  harden)   cmd_harden   ;;
  -h|--help|help) usage ;;
  *) die "Unknown command '$CMD'. Try: ./next-deploy.sh help" ;;
esac
