...
Built from your `next-deploy.<name>.conf` files, so it always matches what's
...
assemble release/                  ← standalone + .next/static + public
...
Only 3 releases are kept, not 5 — a standalone Next.js release is far larger
than a bundle of static files.
...
needed, based on whether any `NEXT_PUBLIC_*` key changed. See
...
```bash
./next-deploy.sh status develop    # pm2 state, live release, port health, disk, memory
./next-deploy.sh logs develop      # pm2 logs — where a crash shows up
./next-deploy.sh restart develop   # restart the process
./next-deploy.sh ssh develop       # shell on the server, in the checkout
```
